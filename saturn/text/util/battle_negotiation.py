...
def compile_item_names(stock: bytes, metrics: FontMetrics) -> bytes:
    prefixes = ("game.itemname.",)
    physical = load_physical_records()
    required = {
        record_id
        for record_id in physical
        if record_id.startswith(prefixes) and record_id.endswith(".name")
    }
    translations = load_bound_translations(prefixes, required_ids=required)
    if len(stock) != 287 * 0x60 or len(translations) != 287:
        raise ValueError("ITEMNAME name inventory changed")
    output = bytearray(stock)
    ranges: list[list[int]] = []
    for record in range(287):
        base = record * 0x60
        words = struct.unpack_from(">41H", stock, base + 0x0C)
        try:
            end = words.index(0x8000)
        except ValueError as error:
            raise ValueError(f"ITEMNAME record {record} has no description terminator") from error
        start = base + 0x0C + (end + 1) * 2
        stop = base + 0x5E
        if any(stock[start:stop]):
            raise ValueError(f"ITEMNAME record {record} has nonzero description padding")
        if start < stop:
            ranges.append([start, stop])

    for record in range(287):
        physical_id = f"game.itemname.o{record * 0x60 + 4:06x}.name"
        text = _normalize(translations[physical_id])
        if "\n" in text or any(not isinstance(token, Text) for token in parse_tokens(text)):
            raise ValueError(f"{physical_id}: item names must be literal single-line text")
        glyphs = metrics.segment(text)
        encoded = bytes(glyph.code for glyph in glyphs)
        width = sum(glyph.advance for glyph in glyphs)
        if len(encoded) > 19 or width > 80 or 0 in encoded or 0xFF in encoded:
            raise ValueError(
                f"{physical_id}: item name exceeds negotiation limits "
                f"({len(encoded)} glyphs, {width}px)"
            )
        payload = encoded + b"\xff"
        offset = next(
            (span[0] for span in ranges if span[1] - span[0] >= len(payload)),
            None,
        )
        if offset is None:
            raise ValueError(f"{physical_id}: ITEMNAME padding is exhausted")
        span = next(span for span in ranges if span[0] == offset)
        span[0] += len(payload)
        output[offset : offset + len(payload)] = payload
        base = record * 0x60
        output[base + 4 : base + 12] = encoded[:8].ljust(8, b"\x00")
        struct.pack_into(">H", output, base + 0x5E, offset)
    return bytes(output)
